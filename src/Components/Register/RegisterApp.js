...
function RegisterApp() {
    const {checkGamerTag, gamerTagAvailable, submitForm, uploadImages, formSubmitted} = useAuth()
    ...
    useEffect(() => {
        if(formSubmitted){
            setCurrentShareLink(window.location.origin + "referral/" + allFieldValues.uniqueGamerId);
            if(width < deltaWidth){
                return <Redirect to={"/verification/" + allFieldValues.uniqueGamerId} />
            }else{
                setActive(4)
            }
        }
        ...
        if(gamerTagAvailable === -1){
            try {
                ...
                if(document.getElementById("uniqueGamerTag")){
                    document.getElementById("uniqueGamerTag").style.border = "1px solid red";
                }else{
            ...
            if(document.getElementById("uniqueGamerTag")){
                document.getElementById("uniqueGamerTag").style.border = "1px solid #fff";
            }
            setActive(2);
        }
    }, [gamerTagAvailable])
    ...
    const inputFieldWrapper = (labelText, subLabelText, fieldName, placeholder) => {
        return (
            <div>
        ...
        return (
            <div>
                ...
                <div className="attach-file-button">
                    ...
                    <input type="file" id={subLabelText+"fileInput"} 
                        style={{visibility:"hidden", display:"none"}} 
                        accept="image/*"
                        onChange={() =>{
                            var oFReader = new FileReader();
                            var attachedFile = document.getElementById(subLabelText+"fileInput").files[0]
                            oFReader.readAsDataURL(attachedFile);
                            setAllFieldValues({...allFieldValues, ["attachedFile" + subLabelText]: attachedFile})

                            oFReader.onload = function (oFREvent) {
                                document.getElementById(subLabelText + "preview").src = oFREvent.target.result;
                            };
                        }}
                        />
                </div>
                <img id={subLabelText + "preview"} src={""} className="preview-attached-image"/>
        ...
        return(
            <div className="register-form">
                <div className="content-wrapper">
                    ...
                    {inputFieldWrapper("Unique Gamer Tag", "(keep it simple and classy)", "uniqueGamerTag","Xx_catDestroyer96_xX")}
                    <div>
                ...
                <div className="next-form-button-wrapper">
                    <button className="next-form-button" 
                        onClick={() => {
                            checkGamerTag(allFieldValues.uniqueGamerTag);
                        }}>Enter Player Details</button>
                </div>
            </div>
        )
    };
    ...
    const playerRegistrationContent= () =>(
        <div className="register-form">
            <div className="content-wrapper">
                ...
                {inputFieldWrapper("College Name", "(include the campus name)", "collegeName","Indian Institute of Technology Bombay")}
                {inputFieldWrapper("College Email", "(this email will be used for verification)", "collegeEmail", "jennytalia@iitb.ac.in")}
                {inputFieldWrapper("Personal Email", "(this email will be used for registration)", "personalEmail", "jennytalia@gmail.com")}
                {inputFieldWrapper("Phone Number", "(For important alerts and prize distribution)", "phoneNumber", "696-969-6969")}
            </div>
            <div className="next-form-button-wrapper">
                {width < deltaWidth 
                ?
                    <button className="back-form-button" onClick={() => setActive(1)}>Back to ‘Gamer Tag’</button>
                : 
                    <button className="next-form-button" onClick={() => setActive(3)}>Attach college photo ID</button>
                }
            </div>
        </div>
    );
    ...
    const attachFilesContent= () =>(
        <div className="register-form">
            ...
            <div className="next-form-button-wrapper">
            <button className="back-form-button" onClick={() => setActive(2)}>{width < deltaWidth ? "Back to ‘Player Details’":"Go Back"}</button>
            <button id="complete-form-button" 
                onClick={async () => {
                    if(ifEmptyAllFields()){
                    ...
                    if(allFieldValues.attachedFileFRONT === '' || allFieldValues.attachedFileBACK == ''){
                        return;
                    }
                    console.log(allFieldValues.attachedFileFRONT.name)

                    await uploadImages(allFieldValues.attachedFileFRONT, 
                        allFieldValues.uniqueGamerId + "__FRONT__" + 
                        allFieldValues.attachedFileFRONT.name)

                    await uploadImages(allFieldValues.attachedFileBACK, 
                        allFieldValues.uniqueGamerId + "__BACK__"+ 
    ...
    const contentsIndex = [
            ...
            {text:"Gamer Tag", title:false, link:"", titleText:"Reserve your Unique Gamer Tag"},
            {text:"Player Details", title:false, link:"", titleText:"Player registration"}, 
